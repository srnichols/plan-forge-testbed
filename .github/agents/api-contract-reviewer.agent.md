...
- **Before reviewing**: `search_thoughts("API contract findings", project: "MyTimeTracker", created_by: "copilot-vscode", type: "convention")` — loads prior API versioning decisions and breaking change history
- **After review**: `capture_thought("API Contract Reviewer: <N findings — key issues>", project: "MyTimeTracker", created_by: "copilot-vscode", source: "agent-api-contract-reviewer")` — persists API contract violations and versioning decisions
