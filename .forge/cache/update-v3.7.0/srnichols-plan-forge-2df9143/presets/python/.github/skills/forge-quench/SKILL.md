---
name: forge-quench
description: "Systematically reduce Python code complexity while preserving exact behavior — measure, understand, propose, prove, report. Use after a feature is complete and tests pass, when code works but is harder to maintain than it should be."
argument-hint: "[optional: specific files or directories to simplify, e.g. 'src/services/' or 'src/services/order_service.py']"
tools:
  - run_in_terminal
  - read_file
  - grep_search
...
Identify the most complex functions in the target files. Use Python-appropriate static analysis:

```bash
# Cyclomatic complexity — grade C or worse (high complexity)
radon cc <target> -s -n C

# Maintainability index — lower scores indicate harder-to-maintain code
radon mi <target>

# Complexity trends over time (if wily is configured)
wily build && wily report <target>

# Count functions exceeding 50 lines
grep -rn "def " <target> --include="*.py" | head -30

# Manual analysis: identify deep nesting, long if/elif chains, excessive branching
grep -rn "if\|elif\|else\|except\|for\|while" <target> --include="*.py" | wc -l
```

List the **top 3–5 most complex functions** by:
- Line count (functions >50 lines)
- Nesting depth (>3 levels of indentation)
- Branch count (>5 if/elif/else branches)
...
2. **Run the full test suite** immediately (`pytest`)
3. **If tests pass**: Commit with a descriptive message:
   ```
   refactor(services): extract order validation into separate function
   
   Chesterton's Fence: <why the complexity existed>
   Simplification: <pattern applied>
   Behavior: unchanged — all N tests pass (`pytest`)
   ```
