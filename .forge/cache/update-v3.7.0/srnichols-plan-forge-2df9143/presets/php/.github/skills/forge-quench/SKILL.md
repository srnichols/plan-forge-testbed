---
name: forge-quench
description: "Systematically reduce PHP code complexity while preserving exact behavior — measure, understand, propose, prove, report. Use after a feature is complete and tests pass, when code works but is harder to maintain than it should be."
argument-hint: "[optional: specific files or directories to simplify, e.g. 'src/Services/' or 'src/Services/OrderController.php']"
tools:
  - run_in_terminal
  - read_file
  - grep_search
...
Identify the most complex functions in the target files. Use PHP-appropriate static analysis:

```bash
# Cyclomatic complexity and code size analysis via PHP Mess Detector
vendor/bin/phpmd src/ text codesize

# Code metrics (lines of code, class count, method count, complexity)
vendor/bin/phploc src/

# Count functions exceeding 50 lines
grep -rn "function " <target> --include="*.php" | head -30

# Manual analysis: identify deep nesting, long switch statements, excessive branching
grep -rn "if\|else\|switch\|case\|catch" <target> --include="*.php" | wc -l
```
...
2. **Run the full test suite** immediately (`vendor/bin/phpunit`)
3. **If tests pass**: Commit with a descriptive message:
   ```
   refactor(services): extract validation from OrderController
   
   Chesterton's Fence: <why the complexity existed>
   Simplification: <pattern applied>
   Behavior: unchanged — all N tests pass (`vendor/bin/phpunit`)
   ```
