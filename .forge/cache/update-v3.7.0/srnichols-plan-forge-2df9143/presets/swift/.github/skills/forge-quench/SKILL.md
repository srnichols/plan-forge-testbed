---
name: forge-quench
description: "Systematically reduce Swift code complexity while preserving exact behavior — measure, understand, propose, prove, report. Use after a feature is complete and tests pass, when code works but is harder to maintain than it should be."
argument-hint: "[optional: specific files or directories to simplify, e.g. 'Sources/Services/' or 'Sources/Services/OrderService.swift']"
tools:
  - run_in_terminal
  - read_file
  - grep_search
...
Identify the most complex functions in the target files. Use Swift-appropriate static analysis:

```bash
# Complexity analysis via SwiftLint
...
grep -rn "func " <target> --include="*.swift" | head -30

# Manual analysis: identify deep nesting depth, long switch statements
grep -rn "if\|else\|switch\|case\|catch\|guard" <target> --include="*.swift" | wc -l
```
...
2. **Run the full test suite** immediately (`swift test`)
3. **If tests pass**: Commit with a descriptive message:
   ```
   refactor(services): extract payment strategy from OrderService
   
   Chesterton's Fence: <why the complexity existed>
   Simplification: <pattern applied>
   Behavior: unchanged — all N tests pass (`swift test`)
   ```
