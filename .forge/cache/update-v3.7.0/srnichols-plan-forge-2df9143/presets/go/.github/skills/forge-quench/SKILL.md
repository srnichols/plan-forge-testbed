---
name: forge-quench
description: "Systematically reduce Go code complexity while preserving exact behavior — measure, understand, propose, prove, report. Use after a feature is complete and tests pass, when code works but is harder to maintain than it should be."
argument-hint: "[optional: specific files or directories to simplify, e.g. 'internal/handlers/' or 'internal/handlers/order.go']"
tools:
  - run_in_terminal
  - read_file
  - grep_search
...
Identify the most complex functions in the target files. Use Go-appropriate static analysis:

```bash
# Cyclomatic complexity — functions exceeding threshold of 10
gocyclo -over 10 .

# Cognitive complexity — functions exceeding threshold of 10
gocognit -over 10 .

# Combined lint with complexity checks
golangci-lint run --enable gocyclo,gocognit

# Count functions exceeding 50 lines
grep -rn "func " <target> --include="*.go" | head -30

# Manual analysis: identify deep nesting, long switch statements, nested error handling
grep -rn "if err\|switch\|case\|select" <target> --include="*.go" | wc -l
```
...
2. **Run the full test suite** immediately (`go test ./...`)
3. **If tests pass**: Commit with a descriptive message:
   ```
   refactor(handlers): flatten nested error handling in CreateOrder
   
   Chesterton's Fence: <why the complexity existed>
   Simplification: <pattern applied>
   Behavior: unchanged — all N tests pass (`go test ./...`)
   ```
