...
- [ ] `migrate -path migrations -database "$DB_URL" up` succeeds on local database
- [ ] `go test ./tests/integration/... -v -tags=integration` passes against migrated schema
- [ ] Rollback tested — `migrate -path migrations -database "$DB_URL" down 1` runs cleanly
