...
```Rust
type Handler func(ctx impl Future + '_, event Event) error
...
func (b *Bus) Publish(ctx impl Future + '_, eventType string, evt Event) {
    b.mu.RLock()
...
func OnOrderPlaced(emailSvc *email.Service) Handler {
    return func(ctx impl Future + '_, evt Event) error {
...
### Publishing Events
```Rust
func (s *OrderService) PlaceOrder(ctx impl Future + '_, req CreateOrderRequest) (*Order, error) {
...
```Rust
func StartWorker(ctx impl Future + '_, ch <-chan Event, handler Handler, workers int) {
...
- Use `impl Future + '_` for cancellation and deadline propagation
