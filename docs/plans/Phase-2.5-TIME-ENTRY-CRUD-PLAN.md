...
> **Ordering note**: This phase was backfilled after the time-entry CRUD surface was found to be
> *assumed by every phase but owned by none*. It must run **after Phase 2 (Projects CRUD)** —
> `TimeEntry.ProjectId` is a foreign key to `Project` — and **before Phase 3 (Invoice Engine),
> Phase 4 (Reports), Phase 5 (Dashboard), and Phase 6 (Web UI)**, all of which read or surface
> time entries.

## Feature Specification: Time Entry CRUD

### Problem Statement
TimeTracker's entire value proposition is logging billable hours, yet there was no owned API surface
to create, list, retrieve, or delete time entries. The Invoice Engine, Reports, Dashboard, and Web UI
...
(`/time-entries`) calls `GET/POST/DELETE api/timeentries`. A prior ad-hoc controller injected
`TimeTrackerDbContext` directly (a Controller → data-access layering violation). This phase makes the
Time Entry CRUD surface a first-class, owned deliverable with proper service-layer separation.
...
- [ ] SHOULD: Service uses `AsNoTracking` for reads
...
- `TimeEntriesController` with GET (list, filtered), GET/{id}, POST, DELETE endpoints depending on the service
...
- [ ] `GET /api/timeentries` returns time entries (the `/time-entries` Web page loads)
...
2. Implement `TimeEntryService` as `sealed class` with primary constructor `(TimeTrackerDbContext dbContext)`, matching the existing service convention
3. Reads use `AsNoTracking`; list ordered by `Date` descending then `Id` descending
4. `CreateAsync` validates hours (0.01–24) and description (≤ 1000), verifies the project exists, stores `Date` as UTC
...
**Validation Gate**: `dotnet build` succeeds; `GET /api/timeentries` returns 200.
